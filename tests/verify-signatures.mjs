 ...
 *
 * Usage:  node tests/verify-signatures.mjs          # verify only
 *         node tests/verify-signatures.mjs --fix    # verify + append uncovered functions
 */
...
async function main() {
  ...
  // --fix mode: append uncovered functions to the skill document
  const fixMode = process.argv.includes("--fix");
  if (fixMode && coverageGaps.length > 0) {
    console.log("\n--- Fix mode: appending uncovered functions ---\n");
    let skillContent = await readFile(SKILL_PATH, "utf-8");

    // Remove any previous auto-generated section
    const autoGenMarker = "<!-- AUTO-GENERATED: Uncovered LIBVLC_API Functions -->";
    const markerIdx = skillContent.indexOf(autoGenMarker);
    if (markerIdx !== -1) {
      skillContent = skillContent.substring(0, markerIdx).trimEnd() + "\n";
    }

    // Group uncovered functions by header file and version availability
    const grouped = { "both": [], "3.x only": [], "4.x only": [] };
    for (const name of allHeaderFuncs) {
      if (skillFuncs.has(name)) continue;
      const in3 = funcs3x.has(name);
      const in4 = funcs4x.has(name);
      const key = in3 && in4 ? "both" : in3 ? "3.x only" : "4.x only";
      const info = (in4 ? funcs4x : funcs3x).get(name);
      grouped[key].push({ name, signature: info.signature, file: info.file });
    }

    let section = `\n\n${autoGenMarker}\n`;
    section += `## Uncovered LIBVLC_API Functions\n\n`;
    section += `> Auto-generated by \`verify-signatures.mjs --fix\`. `;
    section += `These functions exist in VLC headers but are not yet documented in the skill. `;
    section += `Review, add context, and move to the appropriate sections above.\n\n`;

    for (const [label, funcs] of Object.entries(grouped)) {
      if (funcs.length === 0) continue;
      const marker = label === "3.x only" ? " `[3.x]`" : label === "4.x only" ? " `[4.x]`" : "";
      section += `### ${label === "both" ? "Both versions" : label}${marker}\n\n`;

      // Sub-group by header file
      ...
      for (const [file, fileFuncs] of byFile) {
        section += `**${file}**\n\`\`\`c\n`;
    ...
    await writeFile(SKILL_PATH, skillContent + section, "utf-8");
    console.log(`  Appended ${coverageGaps.length} uncovered functions to skill document.\n`);
  }
